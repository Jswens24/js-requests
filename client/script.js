...
const helloChangeColor = () => {
    sayHelloButton.classList.add('sayHelloBlack');
}

sayHelloButton.addEventListener('mouseover', helloChangeColor);
...
const ogHelloColors = () => {
    sayHelloButton.classList.add('sayHelloOg');
}

sayHelloButton.addEventListener('mouseout', ogHelloColors);
    ...
    axios
        ...
        .then(res => {
            for (let i = 0; i < res.data.length; i++) {
                const ohMyText = document.createElement('p');
                const bodyElement = document.querySelector('body')
                ohMyText.textContent = res.data[i];
                bodyElement.appendChild(ohMyText);
            }
        })
        .catch(err => {
            console.log(err)
...
const repeatMyParam = () => {
    axios
        .get('http://localhost:3000/repeat/babyParam')
        .then(res => {
...
const queryTest = () => {
    axios
        .get('http://localhost:3000/query-test/?babyQuery=bigKid')
        .then(res => {
            console.log(res.data);
        })
        .catch(err => console.log(err))
...
/*
    ...
    1: Send no queries on the URL -- what happened? Errors, 'cannot set headers after they are sent to the client'

    2: Send more than 1 query on the URL -- what happened? everything additional was added to the value of the query
*/
